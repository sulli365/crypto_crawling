...
begin
  ...
  select
    ...
    content,
    metadata,
    1 - (crypto_api_site_pages.embedding <=> query_embedding) as similarity
  from crypto_api_site_pages
  where metadata @> filter
  order by crypto_api_site_pages.embedding <=> query_embedding
  limit match_count;
end;
$$;
...
alter table crypto_api_site_pages enable row level security;

-- Create a policy that allows anyone to read
create policy "Allow public read access"
  on crypto_api_site_pages
  for select
  to public
  using (true);